...
const separators = ["break1", "break2", "break3"].map(id =>
...
function changeTheme(themeName) {
    ...
    if (themeWineButton) {
        themeWineButton.src =
            config.button || "/Apollonian-Dionysian/imgs/wine_button.png";
    }
...
window.addEventListener("DOMContentLoaded", () => {
    const sessionTheme =
        sessionStorage.getItem("selectedTheme") || "default";

    changeTheme(sessionTheme);

    // Listener toggle light mode
    if (toggle) {
